...
const Metrics = () => {
  ...
  const { userUSDCBalance, totalAssets, totalSupply, loading, userShareBalance } = useBalances();
  ...
  return (
    <div className="lg:max-w-6xl w-full space-y-2 mx-auto p-4">
      ...
      <div className="flex flex-col py-4 lg:flex-row gap-4">
        ...
        <MetricCard
          title="Your FLH Shares"
          value={userShareBalance}
          unit="FLH"
          isLoading={loading.userShareBalance}
          isConnected={!!connectedAddress}
        />
        <MetricCard
          title="Your USDC Balance"
          value={userUSDCBalance?.value}
          unit="USDC"
          isLoading={loading.userUSDCBalance}
          isConnected={!!connectedAddress}
        />
      </div>
    </div>
...
const MetricCard = ({ title, value, unit, isLoading, isConnected }: any) => {
  const formattedValue = !isLoading ? Number(formatUnits(value || 0, USDC_DECIMALS)).toFixed(2) : null;
  ...
  return (
    <div className="bg-white overflow-hidden shadow-md rounded-lg lg:p-10 py-8 px-20 flex my-2 flex-col items-center">
      ...
      <div className="flex items-center flex-row-reverse">
        {unit === "USDC" && <img src="//logotyp.us/file/usd-coin.svg" alt="USDC" className="h-12 w-12" />}
