...
async function getPoolDetails(poolAddress) {
  ...
  const details = {
    ...
    fee: await new Promise(resolve => {
      poolContract
        .fee()
        .then(f => resolve(f.toString()))
        .catch(() => resolve(null));
    }),
    liquidity: await new Promise(resolve => {
      poolContract
        .liquidity()
        .then(l => resolve(l.toString()))
        .catch(() => resolve(null));
    }),
    slot0: await new Promise(resolve => {
      poolContract
        .slot0()
        .then(s => resolve(s.toString()))
        .catch(() => resolve(null));
    }),
  };
...
async function getPoolAddress(factoryAddress, tokenA, tokenB, fee) {
  ...
  const poolAddress = factoryContract
...
function checkForArbitrage(pools) {
  ...
  validPools.forEach(pool => {
    if (
      pool.price.buyPrice < buyPool.price.buyPrice ||
  ...
  validPools.forEach(pool => {
    if (
      pool !== buyPool &&
      (!sellPool ||
        pool.price.sellPrice > sellPool.price.sellPrice ||
...
async function triggerArbitrageExecution({ buyPool, sellPool, tradeAmount }) {
  try {
    ...
    const tx = await flashVaultContract.executeArbitrage(poolAddress0, poolAddress1, tokenOut, amount, false, {
